...
class LoadImageFromFile_UDA(LoadImageFromFile):
    """Load source and target images together``.
    ...
    def transform(self, results: dict) -> Optional[dict]:
        ...
        ## added by LYU: 2024/04/16
        filename_B = results['B_img_path']
        try:
            if self.file_client_args is not None:
                file_client = fileio.FileClient.infer_client(
                    self.file_client_args, filename_B)
                img_bytes = file_client.get(filename_B)
            else:
                img_bytes = fileio.get(
                    filename_B, backend_args=self.backend_args)
            img = mmcv.imfrombytes(
                img_bytes, flag=self.color_type, backend=self.imdecode_backend)
        except Exception as e:
            if self.ignore_empty:
        ...
        # in some cases, images are not read successfully, the img would be
        # `None`, refer to https://github.com/open-mmlab/mmpretrain/issues/1427
        assert img is not None, f'failed to load image: {filename_B}'
        if self.to_float32:
            img = img.astype(np.float32)

        results['B_img'] = img
        results['B_img_shape'] = img.shape[:2]
        results['B_ori_shape'] = img.shape[:2]
        return results 

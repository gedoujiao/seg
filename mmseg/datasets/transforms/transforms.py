...
class Resize_UDA(MMCV_Resize):
    """Resize source and target images."""
    ...
    def _resize_img(self, results: dict) -> None:
        ...
        if results.get('B_img', None) is not None:
            if self.keep_ratio:
                img, scale_factor = mmcv.imrescale(
                    results['B_img'],
                    results['scale'],
                ...
                img, w_scale, h_scale = mmcv.imresize(
                    ...
                    backend=self.backend)
            results['B_img'] = img
            results['B_img_shape'] = img.shape[:2]
            results['B_scale_factor'] = (w_scale, h_scale)
...
class RandomCrop(BaseTransform):
    ...
    def crop_bbox(self, results: dict) -> tuple:
        ...
        ## added by LYU: 2024/04/17
        if results.get('B_img', None) is not None:
            B_img = results['B_img']
            B_crop_bbox = generate_crop_bbox(B_img)
            return crop_bbox, B_crop_bbox 
    ...
    def transform(self, results: dict) -> dict:
        ...
        ## added by LYU: 2024/04/18
        if results.get('B_img', None) is not None:
            B_img = results['B_img']
            crop_bbox, B_crop_bbox = self.crop_bbox(results)
            B_img = self.crop(B_img, B_crop_bbox)
            results['B_img'] = B_img
            results['B_img_shape'] = B_img.shape[:2]
        else:
